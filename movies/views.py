...
class MovieDetailView(APIView):
    def get(self, request, movie_id):
        ...
        movie_data = {
            ...
            'platform_name'       : [provider.get('provider_name') for provider in provider_data.get('results').get('KR').get('buy')],
            'platform_logo_image' : [TMDB_IMAGE_BASE_URL+provider.get('logo_path') for provider in provider_data.get('results').get('KR').get('buy')],
            'actor'               : [{
                'id'        : actor.get('id'),
                'name'      : actor.get('name'),
                ...
                'role'      : '미구현 제공여부 확인중',
                'role_name' : 'character',
                } for actor in actor_data.get('cast')],  
            'thumbnail_image_url' : TMDB_IMAGE_BASE_URL+movie_data.get('poster_path'),
            'image_url'           : [TMDB_IMAGE_BASE_URL+image.get('file_path') for image in image_data.get('backdrops')],
            'video_url'           : [TMDB_VIDEO_BASE_URL+video.get('key') for video in video_data.get('results')],
            }
